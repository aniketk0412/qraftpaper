...
export default async function BillingPage({
  ...
  // For a real subscription (Solo), fetch the live Lemon Squeezy customer-portal
  // URL so "Manage subscription" opens the actual portal (update card / cancel)
  // instead of starting a second checkout. The one-time trial has no
  // subscription row, so it falls through to the upgrade CTA below.
  const portalUrl =
    isSubscribed && subscription?.lemonSubscriptionId
      ? await getCustomerPortalUrl(subscription.lemonSubscriptionId)
      : null;
  // The $1 3-Day Pass only becomes purchasable once its Lemon Squeezy product
  // exists (one-time, non-subscription). Until LEMONSQUEEZY_VARIANT_TRIAL is
  // set, the Pass renders as "available soon" rather than a checkout that 500s.
    ...
    <div className="min-h-screen lg:pl-[260px]">
      ...
      <main className="px-5 py-8 sm:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          {/* ============================================================ */}
          {/*  PLANS — both plans as equal cards for unpaid visitors;       */}
          {/*  the active plan + live usage for subscribers.                */}
          {/* ============================================================ */}
          <div
            className={cn(
              "mt-12 grid gap-6",
              isSubscribed
                ? "items-start lg:grid-cols-[1.05fr_1fr]"
                : "items-stretch lg:grid-cols-2",
            )}
          >
            ...
            <div className="relative overflow-hidden rounded-3xl glass-strong p-8 ring-1 ring-violet/35 sm:p-10">
              ...
              <div className="relative mt-6">
                {isSubscribed && portalUrl ? (
                  // Active Solo subscriber with a live portal URL: send them to
                  // Lemon Squeezy to update their card, download invoices or
                  // cancel — NOT into a second checkout.
                  <GlowButton
                    href={portalUrl}
                    target="_blank"
                    rel="noreferrer"
          ...
          {/* Value stack sits below the two plan cards for unpaid visitors. */}
          {!isSubscribed && (
            <div className="mt-6">
              <ValueStackCard />
            </div>
...
/* ------------------------------------------------------------------ *
 *  3-Day Pass — the low-commitment plan, shown as a full card beside Solo
 *  so unpaid visitors clearly see TWO plans. Gold-accented to set it apart
 *  from Solo's violet. Reads price/copy from PLANS.trial so it can't drift.
 *  The CTA stays gated ("available soon") until the one-time Lemon Squeezy
 *  product exists; flip `purchasable` by setting LEMONSQUEEZY_VARIANT_TRIAL.
 * ------------------------------------------------------------------ */
function TrialPlanCard({ purchasable }: { purchasable: boolean }) {
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden rounded-3xl border border-gold/35 bg-gold/[0.06] p-8 sm:p-10">
      ...
      <div className="relative mt-7 flex items-end gap-2">
        <span className="text-display-xl text-gradient">{pass.price}</span>
        <span className="mb-2 text-base text-fg-subtle">{pass.period}</span>
      </div>
      <p className="relative mt-2 font-mono text-[0.7rem] uppercase tracking-[0.18em] text-fg-subtle">
        {pass.generationsPerMonth} generations · 3-day access · no subscription
      </p>
      ...
      <ul className="relative mt-5 flex flex-1 flex-col gap-2.5 border-t border-line pt-5 text-[0.84rem] text-fg-muted">
        {pass.features.map((f) => (
          <li key={f} className="flex items-start gap-2.5">
            <span className="mt-0.5 grid h-4 w-4 shrink-0 place-items-center rounded-full bg-gold/15 text-gold">
